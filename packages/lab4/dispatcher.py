...
from ..lr import LRActionDispatcher
...
class Dispatcher(LRActionDispatcher):
    def __init__(self):
        self.reset()
        ...
        for n in ns.children:
            if n.name == ns_name:
                ...
                if n.type != 'ns':
                    ...
                    self.token_counter -= len(ns_name)
                    return False
                else:
                    new_ns = n
                    break
    ...
    def set_parm_name(self, t: str) -> bool:
        s = ''.join(self.buffer)
    ...
    def append_arr_spec(self, p: NSTreeElement) -> bool:
        if self.array_dims:
            # заменяем первый элемент списка размерностей на указатель
            s = '*'+''.join(f'[{x}]' for x in self.array_dims[1:])
            p.type += s
    ...
    def __call__(self, A: str, t: str) -> bool:
        action_map = {'count' : self.count,
                      ...
                      'set_parm_name': self.set_parm_name,
