...
from typing import List, Set, Callable, Iterator, Union
from operator import or_ as union
from functools import reduce


# датакласс для правил
class Rule:
    def __init__(self, left: str, laction: str):
        self.left: str = left
        self.right: List[Union[str, Set[str]]] = []
        # действие для левой части хранится отдельно от остальных
        self.laction: str = laction
...
class Dispatcher(AutomatonActionDispatcher):
    ...
    # добавить пустую цепочку справа
    ...
    def add_range(self, s: str, t: str) -> bool:
        ...
        if self.charset:
            # создать из него строку с первым символом 0x01
            cs = str(self.charset)
            # и добавить её в правило
            r = self.ruleset[-1]
            r.right.append(cs)
            self.charset = CharSet()
            self.prev_left = ''
